...
static int TestRequest1()
{
	...
	auto res = cUrlClient::Get("http://google.com", callbacks, AStringMap(), AString(), options);
	if (res.first)
	{
		evtFinished.Wait();
	}
...
static int TestRequest2()
{
	...
	cEvent evtFinished;
	cCallbacks callbacks(evtFinished);
	auto res = cUrlClient::Get("http://google.com", callbacks);
	if (res.first)
	{
		evtFinished.Wait();
	}
...
static int TestRequest3()
{
	...
	AStringMap options;
	options["MaxRedirects"] = "0";
	auto res = cUrlClient::Get("https://google.com", callbacks, AStringMap(), AString(), options);
	if (res.first)
	{
		evtFinished.Wait();
	}
...
static int TestRequest4()
{
	...
	cEvent evtFinished;
	cCallbacks callbacks(evtFinished);
	auto res = cUrlClient::Get("https://google.com", callbacks);
	if (res.first)
	{
		evtFinished.Wait();
	}
